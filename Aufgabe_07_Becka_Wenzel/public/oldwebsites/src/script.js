...
class MapInterface{
  ...
  addIcons(){
    this.busStopIcon = L.icon({
      iconUrl: 'public/oldwebsites/src/icons/BusStopIcon.png',
      iconSize: [10, 10],
      iconAnchor: [5,5]
    });
  }
...
//##############################################################################
  /**
   * @desc adds a function to visualise the density of busstops as a heatmap
   * @param uses the coordinates of pointcloud to make a new array
   * @returns a layer for the map
   */
  addHeatLayer(){
    ...
    for ( let feature of pointcloud.features){
      coordinatesArray[i] = [feature.geometry.coordinates[1],
        feature.geometry.coordinates[0]]
      i++;
    }
    console.log(coordinatesArray);
    var heat = L.heatLayer(coordinatesArray,{radius: 50}).addTo(this.map);
  }
